...
def transformator(dict_word, sent_word):  # согласование по падежу
    ...
    mot1 = mot1.inflect({mot2.tag.number, mot2.tag.case}).word  # преобразуем словарное по основному
    return mot1
...
for phr_word in phrase:
    ...
    for x in forb_words.keys():  # проходимся по нашему словарю
        ...
        if dist_Lev(init_form, form_2) <= len(form_2) * 0.3:  # не совсем понятно почему, но без нее не работает!!!
